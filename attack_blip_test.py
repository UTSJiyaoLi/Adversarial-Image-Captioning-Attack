...
parser.add_argument('--pixels', type=int, required=False, default=1000, help='Number of pixels to attack.')
...
def display(id, pixel, notes):
    ...
    with Image.open(image_path(id)) as image:
        ...
        # Color to red
        for x, y in pixel:
            image_array[y, x, 0] = 384
        modified_image = Image.fromarray(image_array)
        modified_image.save(f"outputs/blip_flicker8k_1000p_100samples_overall_384/{id}_{notes}.png")
    return 'Finished!'
...
def attack(num_input=args.num_input, 
    ...
    attention_value = AttentionValue(num_input, keywords, 0.5, dir=f'attens_flicker8k_1000samples_384.json', percentage=True)
    ...
    for i in range(num_input):
        ...
        ref_caption = get_coco_dataset(ids[i])[1]
        img, new_img = dataloader.get_item('coco', ids[i], image_size)
        words = blip(image = img, model=model, image_size=image_size)
        bleu_original = bleu4(ref_caption, words)  

        # Different attack settingid, 
        _, best_att_allin, best_pixels_att_allin, caption_att_alllin = differential_evolution_att(ids[i], pixels, index[i], 50, condition='all-in', save_img=save_img,dataset='flicker8k')
        _, best_att_sep, best_pixels_att_sep, caption_att_sep = differential_evolution_att(ids[i], pixels, index[i], 50, condition='separate', save_img=save_img,dataset='flicker8k')
        _, best_noatt, best_pixels_noatt, caption_noatt = differential_evolution_att(ids[i], pixels, index_allin[i], 50, condition='no_att', save_img=save_img,dataset='flicker8k')
        ...
        # Saving demostrasion images, the image after attack is saved in 
        # "differential_evolution_att" function!
        if save_img:
    ...
    # Dumping
    with open(f"outputs/jsons/blip_{pixels}p_{num_input}_overall_384.json", "wb") as file:
